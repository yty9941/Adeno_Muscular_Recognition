...
class ResT(nn.Module):
    def __init__(self, in_chans = config.In_Channels, num_classes= config.Num_Classes, embed_dims=[96, 192, 384, 768],
        ...
        self.head = nn.Linear(embed_dims[3], 10) if num_classes > 0 else nn.Identity()
        self.classfier = nn.Sequential(nn.ReLU(),
                                       nn.Dropout(p = 0.5),
...
def restv2_tiny(pretrained=False, **kwargs):  # 82.3|4.7G|24M -> |3.92G|30.37M   4.5G|30.33M
    model = ResT(embed_dims=[96, 192, 384, 768], depths=[1, 2, 6, 2], **kwargs)
    return model


@register_model
def restv2_small(pretrained=False, **kwargs):  # 83.6|7.0G|35M   -> |5.78G|40.94M
    model = ResT(embed_dims=[96, 192, 384, 768], depths=[1, 2, 12, 2], **kwargs)
    return model


@register_model
def restv2_base(pretrained=False, **kwargs):  # 84.4|10.2G|52M -> |7.25G|55.75M
    model = ResT(embed_dims=[96, 192, 384, 768], depths=[1, 3, 16, 3], **kwargs)
    return model


@register_model
def restv2_large(pretrained=False, **kwargs):  # 85.3|39.6|218M -> |14.09G|98.61M
    model = ResT(num_heads=[2, 4, 8, 16], embed_dims=[128, 256, 512, 1024], depths=[2, 3, 16, 2], **kwargs)
    return model