...
class ResNet_ECA(nn.Module):

    def __init__(self,in_channels, block, layers, num_classes= 6, k_size=[3, 3, 3, 3]):
        self.inplanes = 64
        super(ResNet_ECA, self).__init__()
...
def eca_resnet18(k_size=[3, 3, 3, 3], num_classes= config.Num_Classes, pretrained=False):
    ...
    """
    model = ResNet_ECA(config.In_Channels, ECABasicBlock, [2, 2, 2, 2], num_classes=num_classes, k_size=k_size)
...
def eca_resnet34(k_size=[3, 3, 3, 3], num_classes=1_000, pretrained=False):
    ...
    Args:
        ...
        num_classes:The classes of classification
    """
    model = ResNet_ECA(ECABasicBlock, [3, 4, 6, 3], num_classes=num_classes, k_size=k_size)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model
...
def eca_resnet50(k_size=[3, 3, 3, 3], num_classes=1000, pretrained=False):
    ...
    model = ResNet_ECA(ECABottleneck, [3, 4, 6, 3], num_classes=num_classes, k_size=k_size)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model
...
def eca_resnet101(k_size=[3, 3, 3, 3], num_classes=1_000, pretrained=False):
    ...
    Args:
        ...
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_ECA(ECABottleneck, [3, 4, 23, 3], num_classes=num_classes, k_size=k_size)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model
...
def eca_resnet152(k_size=[3, 3, 3, 3], num_classes=1_000, pretrained=False):
    ...
    Args:
        ...
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_ECA(ECABottleneck, [3, 8, 36, 3], num_classes=num_classes, k_size=k_size)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model
